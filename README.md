...
- Offers an opt-in advanced manual-input recorder for complete prestart-to-gameplay keyboard, mouse, and wheel sequences, with exact-timeline playback and a 10 ms drift safety limit.
...
If a Fast Placement Setup should load a saved Team, also assign the game's **Toggle Unit Inventory** action to a letter, then scroll down to **Controls** on the Dashboard and record the same letter as **Toggle Unit Inventory key**. Leave the setup's Team at **Don't change** when the active team should remain untouched. Legacy camera-model presets retain their own Team fields.

The **Toggle Cancel Unit Placement key** defaults to **Z**. Match it to Anime Expeditions' corresponding binding under **Controls** on the Dashboard. Before every ordinary placement step, the macro selects the intended slot, cancels it, then taps that slot three times to force a deterministic select/deselect/reselect state. The compact **X** beside a game binding unsets it; an unset Cancel Placement key blocks only routes that actually run placement steps, not manual-recording playback. Keep every configured control different from the others.

Set **Change Unit Targeting**, **Upgrade Unit**, **Auto Upgrade Unit**, and **Toggle Auto Upgrade Placed Units** to matching A-Z letters in both Anime Expeditions and **Controls** on the Expeditions Macro Dashboard when a workflow uses them. These controls intentionally start empty and can be unset with their adjacent **X**. Change Unit Targeting is required only for placement steps after First; Auto Upgrade Unit is required only for Priority 1 through Priority 6. Unrelated or manual-recording-only routes do not require unused action keys. Existing **Toggle Auto Upgrade Unit** values migrate to the renamed **Toggle Auto Upgrade Placed Units** control. Every configured letter-based control, Toggle Shift Lock, and the macro hotkey must use a distinct key.
...
4. Set each point's targeting priority and Auto Upgrade priority, set each after-Start point's delay, then click the ordered placement points directly on the native 808 by 611 map screenshot. Auto Upgrade supports Off and Priority 1 through Priority 6, and defaults to Off for new steps. Existing on/off setups migrate on to Priority 1 and off or missing to Off. Use the 50–200% visual zoom when points are close together; scroll over the map to zoom and hold the middle mouse button while dragging to pan. Saved coordinates remain canonical. Points must be at least 7 client pixels apart; right-click a marker to remove it. Drag a row by its grip to reorder it, including edge-scrolling through a long list.
...
6. Changes save automatically. Wait for **All changes saved**, then use **Test playback** to prepare Fast no align and replay the points through the production input path.

Each marker is labeled with its unit slot and `B` or `A` phase. Before-Start rows are always grouped above After-Start rows and can be reordered only within their own phase. Before-Start rows play before the runner deliberately clicks Start; After-Start rows play immediately after the match begins. Playback accepts a placement only after two stable frames show the selected-unit panel, applies the configured targeting priority, then presses Auto Upgrade Unit zero through six times for the step's selected Auto Upgrade priority. The required game-settings check keeps Anime Expeditions' global Auto-Upgrade Placed Units option off so each step remains independently controlled. If the panel is absent, playback keeps the normalized unit slot and retries only a timed 50-pixel approach and click at that coordinate, up to eight attempts.

For an advanced fully manual route, enable **Manual input recordings** in Settings. On the **Recordings** page, arm a recording from the confirmed prestart screen, focus Roblox, press the macro hotkey, manually click Start and perform the run, then press the hotkey again before Victory or Defeat. Physical key presses, mouse buttons, client-relative mouse movement, and vertical or horizontal scrolling are saved against one microsecond timeline. Playback pauses all screen and camera detection, reproduces that sequence from its saved initial pointer position, and stops if any injected event misses its recorded time by more than 10 ms. The runner resumes Victory/Defeat detection only after playback finishes.

Choose the recording from the route's Placement Setup gear. **Prepare recording start** runs the same Fast no-align and deterministic spawn movement used by that route before you record or test it. A per-setup impossibility threshold can override the normal post-playback terminal deadline; zero retains the route default. Recordings are stored locally and may contain sensitive typed input. Their raw events never enter Deep Debug or share codes. A plan whose resolved setup uses a manual recording cannot be exported until **Use manual recording** is turned off for that setup. See [Manual input recordings](docs/MANUAL-INPUT-RECORDINGS.md).
...
5. Use **Export code** to copy the plan, its loop/task structure, any referenced Fast no-align presets, and the exact ordinary Placement Setup overrides or category fallbacks it resolves to as a compact `EMFAST1:` Base64 string. The complete setup includes coordinates, phases, delays, targeting, per-step Auto Upgrade priority, Team, route, default timing, and impossibility threshold. **Import code** validates and restores that dependency-complete bundle on another device. Manual recordings, camera models, app settings, webhooks, private-server links, diagnostics, and task progress are never included.
...
2. Order tasks with **Up** and **Down**. The first eligible row always runs next; remove tasks that should no longer run.
