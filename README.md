...
4. Set each after-Start point's delay in seconds, then click the ordered placement points directly on the native 808 by 611 map screenshot. Points must be at least 7 client pixels apart; right-click a marker to remove it. Drag a row by its grip to reorder it, including edge-scrolling through a long list.
...
Each marker is labeled with its unit slot and `B` or `A` phase. Before-Start rows are always grouped above After-Start rows and can be reordered only within their own phase. Before-Start rows play before the runner deliberately clicks Start; After-Start rows play immediately after the match begins.
