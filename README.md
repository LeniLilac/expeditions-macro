...
Developers can inspect the same captures with the standalone [Detector Viewer](tools/ExpeditionsMacro.DetectorViewer/README.md). It can index every image in the repository dataset corpus with one action, jump to fixtures by filename, and visualize every production detector's owned regions, live action, explicit checks, thresholds, and result, including clear limitations where an internal helper does not expose a safe standalone detail path.
