...
The **Toggle Cancel Unit Placement key** defaults to **Z**. Match it to Anime Expeditions' corresponding binding under **Controls** on the Dashboard. Before every ordinary placement step, the macro selects the intended slot, cancels it, then taps that slot three times to force a deterministic select/deselect/reselect state. Also set **Quick Placement key** to the same physical key used by Anime Expeditions. It starts unset and is required before a plan or test can run any Step Mode rows; Recording Mode routes and empty Step Mode setups do not require it. Playback briefly holds that key and recognizes Anime Expeditions' stable cyan **Quick Placement** title to prove a unit is selected before any coordinate click. Left and right Shift/Ctrl remain distinct. The compact **X** beside either binding unsets it. Keep every configured control different from the others.
...
Each marker is labeled with its unit slot and `B` or `A` phase. Before-Start rows are always grouped above After-Start rows and can be reordered only within their own phase. Before-Start rows play before the runner deliberately clicks Start; After-Start rows play immediately after the match begins. Older setups containing a now-disallowed duplicate Expedition unit or central-HUD coordinate still load for editing; those invalid rows are logged and skipped at runtime. After normalizing a valid configured unit slot once, playback holds Quick Placement and requires the cyan selection title across two stable frames. If it is absent, playback taps the unit slot once more and repeats that bounded proof. A second miss skips the row before any coordinate input. Once selection is proven, playback clicks the coordinate twice directly: once to place and once to select the placed unit. It then parks the pointer so a hover card cannot cover the selected-unit panel and requires that panel across two stable frames. If proof is absent, playback makes at most three slow place/select pairs; each click approaches from 50 client pixels away over 200 milliseconds before clicking the same coordinate. The complete sequence remains bounded to eight coordinate clicks and never repeats unit-key normalization.

A normally confirmed placement receives the configured targeting priority and then zero through six Auto Upgrade Unit presses for Off through Priority 6. If the selected-unit panel is still absent after the eighth coordinate click, the macro logs and skips that placement without targeting, Auto Upgrade, or successful-placement progress, then continues with the next configured step. It never treats an empty Expedition hotbar slot as placement proof because hover and scene changes can make that evidence ambiguous. This local placement miss does not request private-server recovery. The required game-settings check keeps Anime Expeditions' global Auto-Upgrade Placed Units option off so each successfully confirmed step remains independently controlled.
