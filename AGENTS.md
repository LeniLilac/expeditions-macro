...
- Detector ownership must not depend on the exact raster position of a one-pixel decorative line, isolated glyph edge, or other single fragile sample. Search only a bounded field-supported local band, require independent semantic/structural evidence, and retain relevant positive raster-variation plus full cross-state negative coverage. Changing artwork, roster, reward, scene, or page-body brightness may support confidence but must not own a state. OCR or template output may corroborate an independently owned bounded region, but a text or template result alone never authorizes input.
