...
- UI/XAML: relevant view-model/page tests, a Release build, and `scripts/Render-UiSnapshots.ps1 -Configuration Release`; inspect all 68 expected snapshots in both themes, including empty/nested loop plans, the loop-settings and add-task popups, Story Act/Mastery/Infinite option visibility, ready/armed/running Recordings states, the bounded Dashboard run log in wide and compact layouts, the compact stacked Dashboard at the 960 by 640 minimum window, Placement Setup recording settings, the reachable placement-step fields at the minimum window and a 1400 by 1080 medium window, and the collapsed app-navigation/Placement-catalog rails.
