...
- Placement playback attempts: prove the default-unset physical Quick Placement binding, left/right modifier identity, compact X clearing, conflict rejection, persistence, and plan-start resolution across direct and legacy setup dependencies. Recording Mode with preserved steps and an empty Step Mode setup must remain exempt. After normalization, prove one scoped hold with two stable cyan title/icon frames; an initial miss must release the key, tap the unit slot once, and retry once. A second miss must skip before coordinate input and continue the next row. Key-up must survive success, failure, and cancellation. Detector tests must replay selected/unselected field frames and reject every other reviewed dataset image. After selection proof, prove one direct place/select pair with no timed approach, cursor parking before selected-unit capture, up to three slow place/select pairs with a separate acknowledged 50-pixel/200-millisecond approach before each click, an eight-click hard cap without repeated key normalization, and atomic mouse-button release when cancellation interrupts any click. Exhausted proof must log and skip only that row, continue the next step, send no targeting or Auto Upgrade input, omit the successful-placement callback, and avoid runtime/session recovery.
