...
- Placement playback attempts: prove the default-unset physical Quick Placement binding, left/right modifier identity, compact X clearing, conflict rejection, persistence, and plan-start resolution across direct and legacy setup dependencies. Recording Mode with preserved steps and an empty Step Mode setup must remain exempt. For each valid Step Mode row, prove one scoped Quick Placement hold containing at most eight repetitions of exactly one unit-slot tap, one direct saved-coordinate click, cursor parking, and stable selected-unit-panel proof. No Cancel Placement input, slot-normalization preamble, cyan-title detector, or timed mouse approach may run. Stop on the first completed panel proof without an extra tap or click. Key-up must precede targeting and Auto Upgrade and survive success, exhausted proof, click cancellation, and exceptions. Exhausted proof must log and skip only that row, continue the next step, send no targeting or Auto Upgrade input, omit the successful-placement callback, and avoid runtime/session recovery. Atomic mouse-button release must survive cancellation. Authoring tests must prove new map points and recorded captures default to Priority 1 while missing legacy `auto_upgrade` values remain Off.
