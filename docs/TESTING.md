...
- Input, capture, coordinator, or cancellation: focused Windows/workflow tests and the full non-golden suite. Manual-input changes must cover malformed schemas, every supported event type, both wheel directions, ignored hotkey transitions, initial pointer state, delayed native hook callbacks, stable same-millisecond ordering, wrap-safe native timestamps, mixed keyboard/mouse ordering, explicit one-pixel action anchors, incomplete-path rejection before save, the allowed one-pixel client-coordinate rounding variance, rejection beyond one pixel without corrective movement, signed early/late timing-offset boundaries and rejection before and after send, focus/geometry loss, cancellation, and held-input cleanup.
- UI/XAML: relevant view-model/page tests, a Release build, and `scripts/Render-UiSnapshots.ps1 -Configuration Release`; inspect all 72 expected snapshots in both themes, including configured/unset Quick Placement controls and their compact X, empty/nested loop plans, the loop-settings and add-task popups, Story Act/Mastery/Infinite option visibility, ready/armed/running Recordings states, the bounded Dashboard run log in wide and compact layouts, the compact stacked Dashboard at the 960 by 640 minimum window, the Placement Setup Recording Mode / Step Mode selector and its stacked narrow header, Settings > Experimental, the dedicated read-only Settings Diagnostics guidance, the reachable placement-step fields at the minimum window and a 1400 by 1080 medium window, the nested placement-list-to-workspace wheel handoff at both compact scroll boundaries, and the collapsed app-navigation/Placement-catalog rails.
- Placement playback attempts: prove the default-unset physical Quick Placement binding, left/right modifier identity, compact X clearing, conflict rejection, persistence, and plan-start resolution across direct and legacy setup dependencies. Recording Mode with preserved steps and an empty Step Mode setup must remain exempt. After normalization, prove one scoped hold with two stable cyan title/icon frames at both the retained baseline and field-confirmed upward phase; every phase must move the complete structure together without lowering any component gate. An initial miss must release the key, tap the unit slot once, and retry once. A second miss must skip before coordinate input and continue the next row. Key-up must survive success, failure, and cancellation. Detector tests must replay selected/unselected field frames, preserve the independent-icon and broad-cyan negatives, and reject every other reviewed dataset image. After selection proof, prove one direct place/select pair with no timed approach, cursor parking before selected-unit capture, up to three slow place/select pairs with a separate acknowledged 50-pixel/200-millisecond approach before each click, an eight-click hard cap without repeated key normalization, and atomic mouse-button release when cancellation interrupts any click. Exhausted proof must log and skip only that row, continue the next step, send no targeting or Auto Upgrade input, omit the successful-placement callback, and avoid runtime/session recovery.
- Placement authoring and phase editing: cover both Before Start / After Start directions, boundary ordering, unchanged step fields, no-op and invalid requests, autosave persistence, phase-local drag/move behavior, and both themed Placement Setup views. Expedition authoring must reject a unit slot already used in either phase; every mode must reject the fixed central hotbar/HUD rectangle. Legacy duplicate and HUD-conflicting rows must remain loadable/editable, persist until the user changes them, and be skipped individually at runtime rather than invalidating the complete setup. Runtime tests must prove that an empty or flickering Expedition hotbar slot cannot replace selected-unit proof or invoke the successful-placement callback.
