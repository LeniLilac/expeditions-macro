...
- Action: center the cursor, press the Shift Lock key saved under Controls on the Dashboard, then perform pitch and fine-yaw movement while Roblox owns relative pointer motion.
...
- Protected by: `StageHandoffPolicyTests.DifferentModeVictory_UsesTheFieldObservedPlayMenuSequence`, `StageHandoffPolicyTests.LockedRaidSelector_UsesStableSharedPlayEvidenceWithoutRetryingTheKey`, `ExpeditionRunPolicyTests.CompletedRunHandoff_UsesOnlyStateOwnedActions`, `ChallengeScreenDetectorTests.CurrentWideDefeatPanel_WinsOverBackgroundHudControls`, `ChallengeScreenDetectorTests.AnimatedVictoryRewards_RemainStableAcrossConsecutiveFrames`, and Challenge handoff policy tests.
...
- Status: **Product safety contract**.
- Entry: the exact Fast no-align route has a valid local manual recording, the runner has positively verified its prestart state, the client is 808 by 611, and ordinary camera, team, and deterministic route preparation has completed.
- Action: give the recording exclusive Roblox input ownership. Establish its saved initial client-relative pointer through acknowledged motion, start one monotonic clock, and replay physical keyboard scan codes, mouse movement and buttons, and wheel events at their absolute saved offsets. The sequence itself owns the Start Game click.
- Intermediate: do not capture, detect, align the camera, place units, or send any other workflow input while playback owns Roblox.
...
- Do not: begin from an inferred gameplay state, combine a manual recording with ordinary placement steps, teleport the pointer at click time, silently continue after more than 10 ms of event drift, include raw input events in Deep Debug, or include any manual recording or recording reference in a share bundle.
- Failure rule: cancellation, focus/window/geometry change, invalid recording data, input-send failure, or excess timing drift stops playback and releases every held key and mouse button. The incomplete attempt records no task progress. A per-setup impossibility threshold from 1 through 180 minutes may replace the route's default terminal deadline; zero preserves that default.
...
- Protected by: `ManualInputRecordingTests`, `ManualInputWindowsTests`, `ManualInputRouteServiceTests`, `PlacementManualRecordingTests`, `MatchRuntimePolicyTests`, `FastNoAlignShareTests`, and the Recordings/Placement Setup dark-and-light snapshots.

### GB-030: Count completed observations on slow devices

- Status: **Field confirmed** and **product safety contract**.
- Entry: a caller already has a fresh detector result for a Play action, Lobby, Continue action, or mode-owned prestart and asks for consecutive stable observations before input.
- Action: count that initial result, then allow enough fresh detector completions to satisfy the required stability count even when a single detector pass takes longer than the nominal soft wait.
- Exit: act only after the unchanged state and action geometry meet their normal consecutive-observation requirements.
- Do not: discard the caller's valid first observation, measure only wall-clock time while expensive observations are in flight, lower detector confidence, or accept a lone transient frame.
- Failure rule: retain a separate bounded hard deadline and cancellation. If the required completed observations still do not agree, send no input and report the original unavailable-state failure.
...
- Protected by: `ObservationWaitBudgetTests`, stable-navigation tracker tests, Stage/Challenge/Expedition navigation tests, and startup-preflight tests.
