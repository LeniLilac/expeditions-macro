...
- Action: locate the live gray scrollbar thumb three to nine pixels right of the detected red Close-control center, reject candidates outside the field-observed 60–95 pixel height range, and require consecutive matching thumb geometry before acting. If a settled reopening is not at the top, over-drag the detected thumb above the track so Roblox clamps it at the verified top position, then re-verify it. Hold the left mouse button on that thumb and drag it to the requested team's absolute alignment.
