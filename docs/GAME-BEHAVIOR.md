...
- Do not: send accessibility Down/Enter while the confirmation is open; Roblox can move accessibility focus through controls behind the modal instead of confirming it. Do not reuse a terminal close-button coordinate or continue navigation before the modal closure and Lobby are each verified.
...
- Evidence: user-described manual input and passive captures reviewed on 2026-07-22 and 2026-07-26; the latter preserves the accessibility-opened confirmation and the subsequent teleport sequence.
- Protected by: `LobbyExitConfirmationDetectorTests` and `MatchLobbyNavigatorTests.ReturnUsesAccessibilityToOpenThenMouseToConfirm`.
...
- Placement playback: hover the saved target with two acknowledged one-pixel passes, tap its unit slot, send the configured Cancel Placement key, and click that harmless map target without parking the cursor. Tap the unit slot once to establish a known selected state regardless of whether the preceding placement succeeded, hover the target twice again, and settle for 200 milliseconds. Send three ordinary acknowledged placement clicks with 100 milliseconds between attempts. Retain the cursor at the target after attempts one and two, then park only after attempt three, between complete unit-placement sequences.
