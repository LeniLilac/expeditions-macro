...
- Placement authoring: use the exact embedded native 808 by 611 screenshot for the selected category, mode, map, act, or Story run. Store every point with its unit slot and Before Start or After Start phase. After Start points carry independent absolute offsets from the Start click; equal offsets preserve author order for ties. Reject points less than 7 client pixels apart and allow direct marker removal without editing coordinates.
- Placement playback: hover the saved target with two acknowledged one-pixel passes, tap its unit slot, send the configured Cancel Placement key, click that harmless map target, and tap the unit slot once to establish a known selected state regardless of whether the preceding placement succeeded. Hover the target twice again, settle for 200 milliseconds, then use the ordinary jittered click path for the placement itself.
...
- Do not: require scale-dependent Lobby recognition before canonical rendered UI Scale is verified, assume numeric `1.00` is universal, click a page or toggle before scale verification, infer a toggle from label text or a custom font, act on the opening animation, click an unknown control, or begin task navigation from a non-Lobby screen. Disabling automatic correction skips only the Settings work; it does not remove the stable-Lobby start rule.
- Failure rule: use bounded page, toggle, scale, and scrollbar verification. Never click an uncertain control or begin match navigation. A recognized control that ignores input, an unstable panel/Lobby, or a lost/resized Roblox window is a recoverable UI/session failure; rerun the entire preflight after private-server relaunch. An unrecognized control remains a hard compatibility failure.
...
- Resume: require a new Roblox PID and three stable Lobby frames, rerun UI Scale/game-settings preflight, reload the saved plan, and retry the same task. The failed attempt must not increment victories, defeats, runtime target, or completion.
...
- Status: **Field confirmed** for Acts 1–3; Act 4 remains unsupported until its placement capture is available.
...
- Navigation: click the detected Lobby **Events** action, verify the Villain Invasion Event home, click its game-mode action, verify the act selector, choose the configured act, click the detected **Select Stage**, verify the preview, click its live Start action, and wait for verified prestart.
- Transition ownership: Event → Story, Challenge, Expedition, or Raid may use the post-match Play interface. Normal mode → Event and Event → Event must return to verified Lobby first because the Event route is absent from Play.
- Preparation: use Fast no align at the first prestart. Act 1 then holds `W` for 750 ms, `D` for 750 ms, and `W` for 750 ms. Act 2 holds `A` for 75 ms and `W` for 2000 ms. Act 3 needs no spawn movement. Trust these bounded movements and begin state detection again only after they finish.
...
- Runtime rule: Acts 1–3 are 15-wave matches. If no Victory or Defeat appears within 12 minutes after Start, record diagnostics and raise a recoverable session failure so configured private-server recovery can relaunch Roblox and retry the same incomplete task without progress.
- Do not: search Play for Event, send act movement more than once per initial stage load, attempt Act 4 without its placement dataset, or classify Event route pages from labels alone.
- Evidence: reviewed 808 by 611 captures from 2026-07-25 cover the Lobby Event button with and without its notification marker, Villain Invasion home, act selector, Acts 1–3 detail selection, preview, prestart, Victory both with and without **Next Stage**, Defeat, and three Fast no align placement views. Physical act movement timing and Repeat Stage preservation are user-confirmed behavior.
- Protected by: `EventScreenDetectorTests`, `EventRunPolicyTests`, Event placement compatibility checks, current cross-state detector tests, and the Event runner's verified navigation/terminal boundaries.
